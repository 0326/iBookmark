...
const Content: React.FC<ContentProps> = ({ dataSource, refresh }) => {
  ...
  const [selectedBookmark, setSelectedBookmark] = useState<BookmarkNode | null>(
    null
  );
  ...
  const renderListMode = () =>
    list?.map((item) => (
      <section className="w-full divide-y rounded divide-slate-200 ">
        <details
          className="p-4 group border-b-1 border-dashed border-emerald-200"
          open
        >
          <summary className="relative cursor-pointer list-none pr-8 font-medium text-slate-700 transition-colors duration-300 focus-visible:outline-none group-hover:text-slate-900  [&::-webkit-details-marker]:hidden">
            📚{" "}
            {dataSource.title === item.title
              ? `${dataSource.title}`
              : `${dataSource.title} / ${item.title}`}
            <svg
  ...
  const renderTabMode = () => {
    return (
      <section className="m-4" aria-multiselectable="false">
        <ul
          ...
          {list?.map((item, index) => (
            <li role="presentation">
              <button
                ...
                id={`tab-label-${index}f`}
                role="tab"
                aria-setsize="3"
                aria-posinset="1"
                tabindex={current === index ? 0 : -1}
                aria-controls={`tab-panel-${index}f`}
                aria-selected="true"
                onClick={() => {
                  setCurrent(index);
                }}
        ...
        <div>
          {list?.map((item, index) => (
            <div
              className={`px-4 pb-4 text-sm ${current === index ? "" : "hidden"}`}
              id="tab-panel-1f"
              aria-hidden={`${current === index ? "true" : "false"}`}
              role="tabpanel"
              aria-labelledby="tab-label-1f"
              tabIndex={-1}
            >
              <div className="mt-4 text-slate-500 grid grid-cols-4 gap-3 md:grid-cols-3">
                {item.list.map((bookmark) => (
                  <div key={bookmark.id} className="truncate flex items-center">
                    <img
                      src={bookmark.icon}
  ...
  return (
    <>
        ...
        content={
          <div>
            <TextInput
              ...
              // @ts-ignore
              onChange={(value) =>
                setSelectedBookmark({ ...selectedBookmark, title: value })
              }
            />
            <TextInput
              name={i18n("url")}
              ...
              // @ts-ignore
              onChange={(value) =>
                setSelectedBookmark({ ...selectedBookmark, url: value })
              }
            />
          </div>
        }
      ...
      />
      {list?.length > 5 ? renderTabMode() : renderListMode()}
    </>
  );
};
